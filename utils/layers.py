...
class FFLinear(Linear):
    ...
    def __init__(
        ...
        self.optimizer = optimizer(
            self.parameters(), lr=layer_optim_learning_rate
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        x = x / (
            x.norm(2, 1, keepdim=True) + 1e-8
        )  # mormalize input
        return self.activation(
            torch.mm(x, self.weight.T) + self.bias.unsqueeze(0)
        )
    ...
    def reset_parameters(self) -> None:
        ...
        super(FFLinear, self).reset_parameters()
