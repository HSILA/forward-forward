...
class FFSequentialModel(ABC):
    ...
    def train_batch_progressive(
        ...
        """
        for i, layer in tqdm(
            enumerate(self.layers), total=len(self.layers)
        ):
            for epoch in range(epochs):
                for X_pos, X_neg in train_loader_progressive:
                    ...
                    print(
                        f"Epoch: {epoch+1}/{epochs}, Layer {i}: {layer_loss}",
                        end="\r",
                    )
...
class MultiLayerPerceptron(Module):
    ...
    def __init__(
        self, hidden_dimensions: List, activation: torch.nn
    ):
        """Initialize MLP model
        ...
        for i in range(len(hidden_dimensions) - 1):
            self.layers += [
                Linear(
                    hidden_dimensions[i], hidden_dimensions[i + 1]
                )
            ]
